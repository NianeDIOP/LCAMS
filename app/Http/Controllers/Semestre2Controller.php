...
{
    /**
     * Affiche la page d'accueil du semestre 2
     */
    public function index()
    {
        ...
        $distribution = [
            'moins_5' => MoyenneGeneraleS2::where('annee_scolaire_id', $anneeScolaireActive->id)
                ->where('moyenne', '<', 5)->count(),
            '5_10' => MoyenneGeneraleS2::where('annee_scolaire_id', $anneeScolaireActive->id)
                ->whereBetween('moyenne', [5, 9.99])->count(),
            '10_12' => MoyenneGeneraleS2::where('annee_scolaire_id', $anneeScolaireActive->id)
                ->whereBetween('moyenne', [10, 11.99])->count(),
            '12_14' => MoyenneGeneraleS2::where('annee_scolaire_id', $anneeScolaireActive->id)
                ->whereBetween('moyenne', [12, 13.99])->count(),
            '14_16' => MoyenneGeneraleS2::where('annee_scolaire_id', $anneeScolaireActive->id)
                ->whereBetween('moyenne', [14, 15.99])->count(),
            '16_plus' => MoyenneGeneraleS2::where('annee_scolaire_id', $anneeScolaireActive->id)
                ->where('moyenne', '>=', 16)->count(),
        ];
        
        // Moyennes par classe
        $moyennesParClasse = DB::table('moyennes_generales_s2')
            ->join('eleves', 'moyennes_generales_s2.eleve_id', '=', 'eleves.id')
            ->join('classes', 'eleves.classe_id', '=', 'classes.id')
            ->select('classes.libelle as classe', DB::raw('AVG(moyennes_generales_s2.moyenne) as moyenne'))
            ->where('moyennes_generales_s2.annee_scolaire_id', $anneeScolaireActive->id)
        ...
        return view('semestre2.index', compact(
            'anneeScolaireActive', 
            'totalEleves', 
            'totalClasses', 
            'distribution', 
    ...
    public function analyseMoyennes()
    {
        ...
        return view('semestre2.analyse_moyennes', compact('anneeScolaireActive', 'niveaux'));
    }
    ...
    public function getMoyennesClasse($classe_id)
    {
        ...
        $eleves = Eleve::with(['moyenneGeneraleS2'])
    ...
    public function analyseDisciplines()
    {
        ...
        return view('semestre2.analyse_disciplines', compact('anneeScolaireActive', 'niveaux', 'disciplines'));
    }
    ...
    public function getDisciplinesClasse(Request $request)
    {
        ...
        $notes = DB::table('notes_s2')
            ->join('eleves', 'notes_s2.eleve_id', '=', 'eleves.id')
            ->join('disciplines', 'notes_s2.discipline_id', '=', 'disciplines.id')
            ->select(
                'eleves.id',
                'eleves.prenom',
                'eleves.nom',
            ...
            ->where('notes_s2.discipline_id', $discipline_id)
            ->where('notes_s2.annee_scolaire_id', $anneeScolaireActive->id)
            ->orderBy('eleves.nom')
            ->orderBy('eleves.prenom')
            ->get();
        
        // Statistiques
        $statistics = DB::table('notes_s2')
            ->join('eleves', 'notes_s2.eleve_id', '=', 'eleves.id')
            ->select(
                DB::raw('AVG(notes_s2.moy_d) as moyenne'),
                DB::raw('MIN(notes_s2.moy_d) as min'),
                DB::raw('MAX(notes_s2.moy_d) as max'),
                DB::raw('COUNT(CASE WHEN notes_s2.moy_d >= 10 THEN 1 END) as success_count'),
        ...
        $statisticsBySex = DB::table('notes_s2')
            ->join('eleves', 'notes_s2.eleve_id', '=', 'eleves.id')
            ->select(
                'eleves.sexe',
                DB::raw('AVG(notes_s2.moy_d) as moyenne'),
    ...
    public function rapports()
    {
        ...
        return view('semestre2.rapports', compact('anneeScolaireActive', 'niveaux'));
    }
